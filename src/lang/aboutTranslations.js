export const aboutTranslations = {
  es: {
    name: "Clemens Michaël Höberth",
    alias: "Creo",
    intro: "Clemens Michaël Höberth (nacido el 26 de abril de 1991 en Viena, Austria) es un compositor y productor de música electrónica reconocido globalmente bajo el nombre artístico Creo.",
    evolution_title: "Orígenes y Evolución: De Hyperdemented a Creo",
    evolution_text: "La trayectoria de Clemens es una historia de evolución constante: Originalmente, el proyecto nació como un dúo con su hermano Stephan. Tras su retiro, Clemens asumió el liderazgo total. Antes de 2016, operaba como 'Hyperdemented', rastro que aún queda en Newgrounds.",
    identity_title: "Identidad Sonora",
    identity_text: "Su música es una amalgama experimental: fusión orgánica de instrumentos de viento (saxofón) con sintetizadores pesados y una complejidad rítmica influenciada por el Dubstep y Electro House.",
    gd_impact: "Su impacto en Geometry Dash ha sido masivo, con himnos como 'Dimension', 'Nautilus' y 'Carnivores'.",
    community_title: "Comunidad y Creadores",
    community_text: "Como artista independiente bajo su propio sello, permite el uso de su música a creadores de contenido siempre que se atribuya correctamente la autoría."
  },
  en: {
    name: "Clemens Michaël Höberth",
    alias: "Creo",
    intro: "Clemens Michaël Höberth (born April 26, 1991 in Vienna, Austria) is a globally recognized electronic music composer and producer under the stage name Creo.",
    evolution_title: "Origins and Evolution: From Hyperdemented to Creo",
    evolution_text: "Clemens' trajectory is a story of constant evolution: Originally, the project began as a duo with his brother Stephan. After Stephan's departure, Clemens assumed full leadership. Before 2016, it operated as 'Hyperdemented', a name that still remains on Newgrounds.",
    identity_title: "Sonorous Identity",
    identity_text: "His music is an experimental amalgam: an organic fusion of wind instruments (saxophone) with heavy synthesizers and a rhythmic complexity influenced by Dubstep and Electro House.",
    gd_impact: "His impact on Geometry Dash has been massive, with anthems like 'Dimension', 'Nautilus' and 'Carnivores'.",
    community_title: "Community and Creators",
    community_text: "As an independent artist under his own label, he allows content creators to use his music as long as proper attribution is given."
  }
};